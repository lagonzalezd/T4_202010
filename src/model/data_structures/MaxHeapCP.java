...
public class MaxHeapCP<K> implements Iterable<K>, IMaxHeapCP<K>
{
	...
	public MaxHeapCP(int initC, Comparator<K> comparator)
	{
		this.comparator = comparator;
		pq = (K[]) new Object[initC + 1];
		N = 0;
	}
	public MaxHeapCP(Comparator<K> comparator)
	{
		this(1, comparator);
	}
	...
	{
		private MaxHeapCP<K> copy;

		public HeapIterator() {
			if (comparator == null) copy = new MaxHeapCP<K>(size());
			else                    copy = new MaxHeapCP<K>(size(), comparator);
			for (int i = 1; i <= N; i++)
				copy.insert(pq[i]);
		}
